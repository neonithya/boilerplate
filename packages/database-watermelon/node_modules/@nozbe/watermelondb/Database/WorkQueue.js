...
var WriterInterfaceImpl = /*#__PURE__*/function (_ReaderInterfaceImpl) {
  ...
  _proto2.subAction = function subAction(writer) {
    if ('production' !== process.env.NODE_ENV) {
      (0, _common.deprecated)('.subAction()', 'Use .callWriter() / .callReader() instead.');
    }
  ...
  _proto2.batch = function batch(...records) {
    var _this$__workQueue$_db;

    this.__validateQueue();

    return (_this$__workQueue$_db = this.__workQueue._db).batch.apply(_this$__workQueue$_db, records);
  };
  ...
  _proto3.enqueue = function enqueue(work, description, isWriter) {
    ...
    return new Promise(function (resolve, reject) {
      if ('production' !== process.env.NODE_ENV && _this._queue.length) {
        ...
        _common.logger.warn("The ".concat(enqueuedKind, " you're trying to run (").concat(description || 'unnamed', ") can't be performed yet, because there are ").concat(queue.length, " other readers/writers in the queue. Current ").concat(currentKind, ": ").concat(current.description || 'unnamed', ". If everything is working fine, you can safely ignore this message (queueing is working as expected). But if your readers/writers are not running, it's because the current ").concat(currentKind, " is stuck. Remember that if you're calling a reader/writer from another reader/writer, you must use callReader()/callWriter(). See docs for more details."));

...
var LokiJSAdapter = /*#__PURE__*/function () {
  function LokiJSAdapter(options) {
    ...
    if ('production' !== process.env.NODE_ENV) {
      ...
      (0, _invariant.default)(!('indexedDBSerializer' in options), 'LokiJSAdapter `indexedDBSerializer` option is now `{ extraIncrementalIDBOptions: { serializeChunk, deserializeChunk } }`');
      (0, _invariant.default)(!('onIndexedDBFetchStart' in options), 'LokiJSAdapter `onIndexedDBFetchStart` option is now `extraIncrementalIDBOptions: { onFetchStart }`');
      (0, _invariant.default)(!('onIndexedDBVersionChange' in options), 'LokiJSAdapter `onIndexedDBVersionChange` option is now `extraIncrementalIDBOptions: { onversionchange }`');
      (0, _invariant.default)(!('autosave' in options), 'LokiJSAdapter `autosave` option is now `extraLokiOptions: { autosave }`');
  ...
  _proto._debugDignoseMissingRecord = function _debugDignoseMissingRecord(table, id) {
    ...
    if (driver) {
      var lokiCollection = driver.loki.getCollection(table); // if we can find the record by ID, it just means that the record cache ID was corrupted

...
var encodeIndex = function (column, tableName) {
  return column.isIndexed ? "create index \"".concat(tableName, "_").concat(column.name, "\" on \"").concat(tableName, "\" (\"").concat(column.name, "\");") : '';
};
...
var encodeTableIndicies = function ({
  ...
  return Object.values(columns) // $FlowFixMe
  .map(function (column) {
    return encodeIndex(column, tableName);
  }).concat(["create index \"".concat(tableName, "__status\" on \"").concat(tableName, "\" (\"_status\");")]).join('');
};
...
var encodeSchema = function ({
  ...
  var sql = Object.values(tables) // $FlowFixMe
...
function encodeDropIndices({
  ...
  .map(function ({
    ...
    return Object.values(columns) // $FlowFixMe
    .map(function (column) {
      return column.isIndexed ? "drop index \"".concat(tableName, "_").concat(column.name, "\";") : '';
    }).concat(["drop index \"".concat(tableName, "__status\";")]).join('');
  }).join('');

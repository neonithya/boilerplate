...
var SQLiteAdapter = /*#__PURE__*/function () {
  function SQLiteAdapter(options) {
    ...
    this._dispatcherType = (0, _makeDispatcher.getDispatcherType)(options); // Hacky-ish way to create an object with NativeModule-like shape, but that can dispatch method
    // calls to async, synch NativeModule, or JSI implementation w/ type safety in rest of the impl

    this._dispatcher = (0, _makeDispatcher.makeDispatcher)(this._dispatcherType, this._tag, this.dbName, usesExclusiveLocking);

    if ('production' !== process.env.NODE_ENV) {
      (0, _common.invariant)(!('experimentalUseJSI' in options), 'SQLiteAdapter `experimentalUseJSI: true` has been renamed to `jsi: true`');
      (0, _common.invariant)(!('synchronous' in options), 'SQLiteAdapter `synchronous: true` was removed. Replace with `jsi: true`, which has the same effect, but with a more modern implementation');
  ...
  _proto.queryIds = function queryIds(query, callback) {
    ...
    this._dispatcher.call('queryIds', // $FlowFixMe
  ...
  _proto.unsafeQueryRaw = function unsafeQueryRaw(query, callback) {
    ...
    this._dispatcher.call('unsafeQueryRaw', // $FlowFixMe
    (0, _encodeQuery.default)(query), callback);
  };
  ...
  _proto.count = function count(query, callback) {
    ...
    this._dispatcher.call('count', // $FlowFixMe
  ...
  _proto.unsafeLoadFromSync = function unsafeLoadFromSync(jsonId, callback) {
    if ('jsi' !== this._dispatcherType) {
      callback({
        error: new Error('unsafeLoadFromSync unavailable')
      });
      return;
    }
    ...
    this._dispatcher.call('unsafeLoadFromSync', [jsonId, schema, encodeDropIndices(schema), encodeCreateIndices(schema)], function (result) {
      return callback((0, _Result.mapValue)( // { key: JSON.stringify(value) } -> { key: value }
  ...
  _proto.provideSyncJson = function provideSyncJson(id, syncPullResultJson, callback) {
    if ('jsi' !== this._dispatcherType) {
      callback({
        error: new Error('provideSyncJson unavailable')
      });
      return;
    }

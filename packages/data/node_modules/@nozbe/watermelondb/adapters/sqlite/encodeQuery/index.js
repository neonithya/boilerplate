...
var encodeWhereCondition = function (associations, table, left, comparison) {
  var {
    operator: operator
  } = comparison; // if right operand is a value, we can use simple comparison
  // if a column, we must check for `not null > null`

  if ('weakGt' === operator && comparison.right.column) {
    return encodeWhere(table, associations)(Q.or( // $FlowFixMe
...
var encodeQuery = function (query, countMode = false) {
  ...
  } = query; // TODO: Test if encoding a `select x.id from x` query speeds up queryIds() calls

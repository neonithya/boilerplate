...
function recordsToApplyRemoteChangesTo_replacement(collection, changes, context) {
  return new Promise(function ($return, $error) {
    ...
    return Promise.resolve(Promise.all([unsafeFetchAsRaws(collection.query.apply(collection, (0, _toConsumableArray2.default)(queryForReplacement ? [Q.or(Q.where((0, _Schema.columnName)('id'), Q.oneOf(idsForChanges(changes))), Q.and.apply(Q, (0, _toConsumableArray2.default)(queryForReplacement)))] : []))), db.adapter.getDeletedRecords(table)])).then(function ($await_4) {
      try {
        ...
        return Promise.resolve(function () {
          return new Promise(function ($return, $error) {
            ...
            if (queryForReplacement) {
              modifiedQuery = collection.query.apply(collection, (0, _toConsumableArray2.default)(queryForReplacement));
...
var applyAllRemoteChanges = function (recordsToApply, context) {
  ...
  (0, _fp.toPairs)(recordsToApply).forEach(function ([tableName, records]) {
    allRecords.push.apply(allRecords, (0, _toConsumableArray2.default)(prepareApplyRemoteChangesToCollection(records, db.get(tableName), context)));
...
var unsafeApplyAllRemoteChangesByBatches = function (recordsToApply, context) {
  ...
  (0, _fp.toPairs)(recordsToApply).forEach(function ([tableName, records]) {
    ...
    var batches = (0, _splitEvery.default)(5000, preparedModels).map(function (recordBatch) {
      return db.batch.apply(db, (0, _toConsumableArray2.default)(recordBatch));
    });
...
function applyRemoteChanges(remoteChanges, context) {
  return new Promise(function ($return, $error) {
    ...
    return Promise.resolve(getAllRecordsToApply(remoteChanges, context)).then(function ($await_7) {
      try {
        recordsToApply = $await_7;
